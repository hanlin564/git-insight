...
| `test/git/repositoryDiscovery.test.ts` | 递归发现 `.git` 目录和 `.git` 文件形式的仓库，支持多个扫描目录，跳过 `node_modules`，不扫描已发现仓库内部。 | 返回正确仓库路径且不重复。 |
| `test/analysis/myGitData.test.ts` | 在临时工作区创建多个 Git 仓库，并隔离 Git 全局用户配置，覆盖多个扫描目录和重复目录。 | 只统计匹配全局用户名或邮箱的提交，正确生成年度和过去 12 个月热力图，以及今天/7 天/30 天摘要。 |
| `test/ui/contributionHeatmap.test.ts` | 验证年度热力图星期行和周数据布局。 | 7 天数据完整保留，界面只隐藏部分行标签而不隐藏日期格子。 |
| `test/cli.test.ts` | 运行编译后的 `dist/index.js`。 | 默认扫描当前目录；支持传入多个相对或绝对扫描目录；缺少全局 Git 用户时报错；无仓库时输出空状态。 |
