...
- 用户名忽略大小写后相同会合并，例如 `Ray-ux` 和 `ray-ux`。
- 邮箱只做首尾空格清理后的精确匹配，大小写不同视为不同邮箱。

如果同一个用户的用户名和邮箱都不同，可以在被分析仓库根目录创建 `.git-insight.json` 补充合并关系。文件不存在时只使用自动合并规则；文件存在但 JSON 格式错误时会输出错误。
